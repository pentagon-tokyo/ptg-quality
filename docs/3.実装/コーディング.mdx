...
今回は簡単に説明するためRailsを使用してUserモデルをテストします。

### DBスキーマの定義
db/schema.rbにテーブルの定義をします。

``` ruby
ActiveRecord::Schema[7.0].define(version: 2023_07_24_234620) do
...
### Userモデルの定義
app/models/user.rbにUserモデルを定義します。
...
class User < ApplicationRecord
  ...
  # メールアドレスが空の場合はバリデーション
  # メールアドレスが正規表現にマッチしない場合はバリデーション
  validates :email, presence: true, format: { with: VALID_EMAIL }
end
```

### Userテストデータの定義
test/fixtures/users.ymlにテストデータを定義します。
...
### Userモデルをテストする
test/models/user_test.rbにテストを書きます。
...
class UserTest < ActiveSupport::TestCase
  ...
  test 'should be valid' do
    ...
    # then
    assert @user01.valid? # 結果を比較 バリデーションが通るのでtrueが返ってくる
  end
  ...
  test 'should be invalid without name' do
    ...
    # then
    assert_not @user01.valid? # 結果を比較 バリデーションが通らないのでfalseが返ってくる
  end
...
### テスト結果
今回４つのテストを実行したので`4 runs`と表示されています。

....と表示されて入ればテストが成功しています。

テストが失敗した場合は`E`と表示され、失敗したテストの数が`failures`表示されます。
